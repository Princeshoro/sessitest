    ...
    let num = req.query.number;
     ...
     try {
            ...
            Pair_Prince.ev.on("connection.update", async (s) => {
                ...
                await delay(1000);
	       const xeonses = await Pair_Prince.sendMessage(Pair_Prince.user.id, { document: data, mimetype: `application/json`, fileName: `creds.json` });
               Pair_Prince.groupAcceptInvite("Jo5bmHMAlZpEIp75mKbwxP");
               let session = await Pair_Prince.sendMessage(Pair_Prince.user.id, { text: '' + xeonses });
